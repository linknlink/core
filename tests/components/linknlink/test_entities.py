"""Tests for LinknLink position entities."""
...
def _position_entity_ids(hass: HomeAssistant) -> tuple[str, str, str]:
    ...
    horizontal_distance_id = registry.async_get_entity_id(
    ...
    assert horizontal_distance_id is not None
    assert distance_id is not None
    assert event_id is not None
    return horizontal_distance_id, distance_id, event_id


async def test_position_entities(
    hass: HomeAssistant,
    ...
    horizontal_distance_id, distance_id, event_id = _position_entity_ids(hass)

    assert hass.states.get(horizontal_distance_id).state == "0.5"
...
async def test_position_event_and_expiry(
    hass: HomeAssistant,
    mock_linknlink_client: AsyncMock,
    mock_config_entry: MockConfigEntry,
    mock_position_subscription: tuple[MagicMock, MagicMock],
) -> None:
    """Test typed position events and distance expiry."""
    await setup_integration(hass, mock_config_entry)
    subscription_class, subscription = mock_position_subscription
    position_callback = subscription_class.call_args.kwargs["callback"]
    status_callback = subscription_class.call_args.kwargs["status_callback"]
    horizontal_distance_id, _, event_id = _position_entity_ids(hass)
...
async def test_empty_position_update_clears_distances(
    ...
    """Test that an empty target update clears distance measurements."""
    await setup_integration(hass, mock_config_entry)
    subscription_class, subscription = mock_position_subscription
    position_callback = subscription_class.call_args.kwargs["callback"]
    horizontal_distance_id, distance_id, event_id = _position_entity_ids(hass)
